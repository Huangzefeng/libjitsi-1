    ...
    implements PushBufferStream
{
    ...
    void setInStreams(Collection<InStreamDesc> inStreams)
    {
        ...
        if (valueIsChanged)
        {
            ...
            {
                SourceStream inStream = inStreamDesc.getInStream();
                logger.debug("Handling inputStream " + inStream.hashCode());

                if (!(inStream instanceof PushBufferStream))
                {
                    logger.debug("inputStream " + inStream.hashCode() + " is not a PushBufferStream");
                    continue;
                }
                if (!skippedForTransferHandler)
                {
                    logger.debug("inputStream " + inStream.hashCode() + " setting skippedForTransferHandler");
                    skippedForTransferHandler = true;
                    continue;
                }
                if (!(inStream instanceof CachingPushBufferStream))
                {
                    logger.debug("inputStream " + inStream.hashCode() + " isn't a CachingPushBufferStream so wrapping");
