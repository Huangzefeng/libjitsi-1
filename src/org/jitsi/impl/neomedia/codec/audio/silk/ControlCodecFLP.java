...
//    //    #else
    ...
    //    #endif
        ...
        if( psEnc.sCmn.fs_kHz != fs_kHz || psEnc.sCmn.prev_API_fs_Hz != API_fs_Hz )
        {
            ...
            SigProcFLP.SKP_short2float_array( psEnc.x_buf,0, x_bufFIX,0, 2 * Define.MAX_FRAME_LENGTH + Define.LA_SHAPE_MAX );
        }
...
//            SKP_memset( psEnc->sCmn.LBRR_buffer, 0,           MAX_LBRR_DELAY * sizeof( SKP_SILK_LBRR_struct ) );
            for(int i=0; i < Define.MAX_LBRR_DELAY; i++)
            {
...
//            SKP_memset( psEnc->sPred.prev_NLSFq, 0, MAX_LPC_ORDER * sizeof( SKP_float ) );
            Arrays.fill(psEnc.sPred.prev_NLSFq, 0, Define.MAX_LPC_ORDER, 0);
