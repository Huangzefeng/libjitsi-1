...
public final class SQLiteConnectionPool implements Closeable {
    private static final Logger sLog = Logger.getLogger(SQLiteConnectionPool.class);
    ...
    private void dispose(boolean finalized) {
        if (!finalized) {
            ...
            synchronized (mLock) {
                ...
                if (pendingCount != 0) {
                    sLog.info("The connection pool for " + mConfiguration.label
    ...
    private boolean recycleConnectionLocked(SQLiteConnection connection,
        ...
        if (status == AcquiredConnectionStatus.RECONFIGURE) {
            try {
                ...
                sLog.error("Failed to reconfigure released connection, closing it: "
                        + connection, ex);
    ...
    void onConnectionLeaked() {
        ...
        sLog.warn("A SQLiteConnection object for database '"
    ...
    private void closeConnectionAndLogExceptionsLocked(SQLiteConnection connection) {
        try {
            ...
            sLog.error("Failed to close connection, its fate is now in the hands "
    ...
    private void reconfigureAllConnectionsLocked() {
        if (mAvailablePrimaryConnection != null) {
            try {
                ...
                sLog.error("Failed to reconfigure available primary connection, closing it: "
        ...
        for (int i = 0; i < count; i++) {
            ...
            try {
                ...
                sLog.error("Failed to reconfigure available non-primary connection, closing it: "
                        + connection, ex);
                closeConnectionAndLogExceptionsLocked(connection);
    ...
    private void finishAcquireConnectionLocked(SQLiteConnection connection, int connectionFlags) {
        try {
            ...
            sLog.error("Failed to prepare acquired connection for session, closing it: "
