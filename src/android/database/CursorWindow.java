...
public class CursorWindow extends SQLiteClosable {
    private static final Logger sLog = Logger.getLogger(CursorWindow.class);
    ...
    public CursorWindow(String name) {
        ...
        if (sCursorWindowSize < 0) {
            /** The cursor window size. resource xml file specifies the value as 2048 kB.
    ...
    private static final HashMap<Long, Integer> sWindowToPidMap = new HashMap<Long, Integer>();

    private void recordNewWindow(int pid, long window) {
        synchronized (sWindowToPidMap) {
            sWindowToPidMap.put(window, pid);
            sLog.trace("Created a new Cursor. " + printStats());
        }
    }
    ...
    private String printStats() {
        ...
        synchronized (sWindowToPidMap) {
            ...
            for (Integer pid : sWindowToPidMap.values()) {
                int value = pidCounts.get(pid);
                pidCounts.put(pid, ++value);
            }
        }
        int numPids = pidCounts.size();
        for (Map.Entry<Integer,Integer> e : pidCounts.entrySet()) {
